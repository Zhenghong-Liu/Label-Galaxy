//import z01 from './z=0.1.json'
//import z02 from './z=0.2.json'
...
//  var path = './z='+z[i]+'.json'
    var path = '../pagedata/gmm/z=0.1.json'
    console.log(path)
...
function getBasicData(callback){
  var i=0
  var path = './z='+z[i]+'.json'
//var path = '@/components/pagedata/gmm/z=0.2.json'
  console.log(path)
//fetch(path).then((res)=>{if (!res.ok) {
//    throw new Error
//        (`HTTP error! Status: ${res.status}`);
...
function recordClickPosition(callback, gaussians) {
  determinator = 1
  ...
  mychart.getZr().on('click', function click_a(params) {
    ...
    callback(point, gaussians)
    // Add your logic here to record the click position
  });
}

function generateGaussian(point, gaussians){
  if (determinator != 1){
    return
  }
...
function displayGaussian(gaussian_series, number){
  ...
  for (var i=0; i<x.length; i++){
    y0.push(gaussian_series.pdf(x[i]))
  }
  var xy0 = new Array()
  for (var i=0; i<x.length; i++){
    xy0.push([x[i],y0[i]])
...
export {getRandomData, displayRandomData, getBasicData, displayBasicData, recordClickPosition,
generateGaussian, select_line, test, refresh, Gaussian_series, displayGaussian}
